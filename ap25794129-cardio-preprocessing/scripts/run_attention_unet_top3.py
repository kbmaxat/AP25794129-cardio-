"""Second-architecture check: Attention U-Net on the top-3 modes (none, wavelet, nlm) across
all three benchmark settings (ACDC, CAMUS, combined), at the same primary 10-epoch budget as the
original UNet2D benchmark. Purpose: test whether the "no preprocessing benefit" finding is a
UNet2D-specific artifact or holds under a different backbone. Deliberately scoped to top-3 modes
rather than all six, matching the same top-3 follow-up pattern already used elsewhere in this
benchmark, to keep this a targeted architecture check rather than a second full study.
...
SESSION_ROOT = Path("outputs/attention_unet_top3")
...
def main() -> None:
    ...
    for group_name, dataset_filter in DATASET_GROUPS.items():
        for mode in MODES:
            jobs.append(
                dict(
                    output_dir=SESSION_ROOT / group_name / f"attn_unet_{group_name}_{mode}",
                    mode=mode,
